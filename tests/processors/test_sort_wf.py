...
from dspeed.processors import sort_wf


def test_sort_wf(compare_numba_vs_python):
    """Testing function for the sort_wf processor."""

    # test basic sorting functionality
    w_in = np.array([5.0, 2.0, 8.0, 1.0, 9.0, 3.0])
    w_out_expected = np.array([1.0, 2.0, 3.0, 5.0, 8.0, 9.0])
    assert np.allclose(compare_numba_vs_python(sort_wf, w_in), w_out_expected)
    ...
    assert np.all(np.isnan(compare_numba_vs_python(sort_wf, w_in)))
