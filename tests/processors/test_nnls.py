...
def test_nnls(compare_numba_vs_python):   
    ...
    optimize_nnls(A,yd,1000,1e-9,False,kernel)
    ...
    assert np.all(np.sort(x[np.where((kernel>0.01))]).astype(int) == np.sort(list(set(mean))))
    assert np.sort(x[np.where((kernel>0.01))]).astype(int)[0] == mean[0]

    # compare numba and python results
    kernel = np.zeros(len(x))
    compare_numba_vs_python(optimize_nnls,A,yd,1000,1e-9,False,kernel)
