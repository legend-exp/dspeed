...
def test_iir_filter(compare_numba_vs_python):
    ...
    for ftype in ["butter", "cheby1", "cheby2", "ellip", "bessel"]:
        for btype in ["lowpass", "highpass", "bandpass", "bandstop"]:
            ...
            # Same filter as above, contstruct with units
            filt = iir_filter(freq_c, order, rp, rs, f_samp=f_samp, btype=btype, ftype=ftype)
            compare_numba_vs_python(filt, w_in, w_out)
            assert np.allclose(w_out, exp_out)
            ...
            if btype in ("lowpass", "highpass"):
                with pytest.raises(DSPFatal):
                    filt = iir_filter([f_c1, f_c2], order, rp, rs, btype=btype, ftype=ftype)
                with pytest.raises(DSPFatal):
                ...
                with pytest.raises(DSPFatal):
                    filt = iir_filter([1.1, 1.2], order, rp, rs, btype=btype, ftype=ftype)
