...
def test_inject_damped_oscillation(compare_numba_vs_python):
    ...
    def test(wf_len: int, t_offset:int, amp: float, tau:float, omega:float, phase:float, frac:float):
        ...
        exp[t_offset:] = frac*amp*np.exp(-np.arange(wf_len-t_offset)/tau)*np.cos(np.arange(wf_len-t_offset)*omega + phase)
        exp += step

        assert( np.max(np.abs( out - exp )) < 1.e-6 )

    test(1000, 100, 10, 100, 0.1, 0, 1.)
    test(1000, 100, 10, 100, 0.1, 0, 0.)
