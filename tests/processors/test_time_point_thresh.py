...
def test_time_point_thresh_nopol(compare_numba_vs_python):
    ...
    with pytest.raises(DSPFatal):
        w_in = np.concatenate([np.arange(-1, 5, 1), np.arange(-1, 5, 1)], dtype="float")
        time_point_thresh_nopol(w_in, 1, 10.5, 0, np.array([0.0]))

    # test for error if walk_forward non integer
    with pytest.raises(DSPFatal):
        w_in = np.concatenate([np.arange(-1, 5, 1), np.arange(-1, 5, 1)], dtype="float")
        time_point_thresh_nopol(w_in, 1, 11, 0.5, np.array([0.0]))

    # test for error if t_start out of range
    with pytest.raises(DSPFatal):
        w_in = np.concatenate([np.arange(-1, 5, 1), np.arange(-1, 5, 1)], dtype="float")
        time_point_thresh_nopol(w_in, 1, 12, 0, np.array([0.0]))

    # test walk backward - finds first point where w_in[i-1] < threshold
    w_in = np.concatenate([np.arange(-1, 5, 1), np.arange(-1, 5, 1)], dtype="float")
    # w_in = [-1, 0, 1, 2, 3, 4, -1, 0, 1, 2, 3, 4], threshold=1, start=11
    # Walking backward from index 11, find first i where w_in[i-1] < 1
    # i=11: w_in[10]=3 < 1? No
    # i=10: w_in[9]=2 < 1? No
    # i=9: w_in[8]=1 < 1? No
    ...
    # These tests use waveforms where time_point_thresh_nopol (without polarity check)
    # produces different results than time_point_thresh (with polarity check).
    ...
    # Test walk forward with no crossing - all values above threshold
    # time_point_thresh_nopol returns nan when no point is at or below threshold
    w_above = np.array([5.0, 6.0, 7.0, 8.0, 9.0])
    assert np.isnan(
        compare_numba_vs_python(time_point_thresh_nopol, w_above, 2.5, 0, 1)
    )

    # Test walk backward with no crossing - all values above threshold
    w_above = np.array([5.0, 6.0, 7.0, 8.0, 9.0])
    assert np.isnan(
        compare_numba_vs_python(time_point_thresh_nopol, w_above, 2.5, 4, 0)
    )
