...
from dspeed.processors import convolve_exp, convolve_damped_oscillator
...
def test_convolve_exp(compare_numba_vs_python):
    ...
    def test_with_delta(wf_len: int, t_offset:int, amp: float, tau:float):
        ...
        assert( np.max(np.abs( out - exp )) < 1.e-6 )
    ...
    def test_with_step(wf_len: int, t_offset:int, amp: float, tau:float):
        ...
        exp[t_offset:] = amp*(1-np.exp(-np.arange(wf_len - t_offset)/tau))/(1-np.exp(-1/tau))
...
def test_convolve_damped_oscillator(compare_numba_vs_python):
    ...
    def test_with_delta(wf_len: int, t_offset:int, amp: float, tau:float, omega:float, phase:float):
        delta = np.zeros(wf_len, 'float32')
        delta[t_offset] = amp
        out = convolve_damped_oscillator(delta, tau, omega, phase)

        exp = np.zeros_like(delta)
        exp[t_offset:] = amp*np.exp(-np.arange(wf_len - t_offset)/tau)*np.cos(np.arange(wf_len - t_offset)*omega + phase)
    ...
    def test_with_step(wf_len: int, t_offset:int, amp: float, tau:float, omega:float, phase:float):
        step = np.zeros(wf_len, 'float64')
        step[t_offset+1:] = amp
        out = convolve_damped_oscillator(step, tau, omega, phase)

        exp = np.zeros_like(step)
        exp[t_offset:] = np.real(amp*np.exp(phase*1j)*(1 - np.exp(np.arange(wf_len-t_offset)*(-1/tau + omega*1j)))/(1-np.exp(-1/tau+omega*1j)))
